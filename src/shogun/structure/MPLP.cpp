...
void CEdgeFactor::update_messages(vector<SGNDArray<float64_t>> &msgs_all_factors)
{
	...
	for (int32_t vi = 0; vi < m_var_index.size(); vi++)
	{
		...
		inds_on_edge[0] = vi;
		SGNDArray<float64_t> arr_expand(orig_energy.get_dims());
	...
	for (int32_t vi = 0; vi < m_var_index.size(); vi++)
	{
		...
		SGVector<int32_t> inds_on_edge(1);
		inds_on_edge[0] = vi;
		SGNDArray<float64_t> arr_expand(msgs_all_factors[m_ind_factor].get_dims());
...
void CMPLP::init()
{
	...
	for (int32_t vi = 0; vi < fg_var_sizes.size(); vi++)
	{
		...
		m_msgs_all_factors.push_back(SGNDArray<float64_t>(subset_size) = 0);
	}
